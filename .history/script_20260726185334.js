...
function drawConnections(canvas, svg) {
  ...
  const traverse = (person) => {
    ...
    person.children.forEach((child) => {
      ...
      line.setAttribute("x1", String(parentPosition.x + 240 - viewOffset.x));
      line.setAttribute("y1", String(parentPosition.y + 70 - viewOffset.y));
      line.setAttribute("x2", String(childPosition.x - viewOffset.x));
      line.setAttribute("y2", String(childPosition.y + 70 - viewOffset.y));
...
function handlePointerMove(event) {
  ...
  if (interactionState.type === "card") {
    ...
    viewOffset = {
      x: interactionState.startOffsetX + (event.clientX - interactionState.startX),
      y: interactionState.startOffsetY + (event.clientY - interactionState.startY),
    };
